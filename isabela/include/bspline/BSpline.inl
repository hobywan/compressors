...
template<class T> BSpline<T>::BSpline(const T *x,
	...
	BSplineBase<T>(x, nx, wl, bc_type, num_nodes), s(new BSplineP<T>)
	{
		//std::cout << "Here.. in Bspline constructor"<< std::endl;
		OK=true;
		s->A.clear();
		for(int i =0 ; i < num_nodes; i++)
		{
			s->A.push_back(coeff[i]);
		}
		this->mean= mean;
	}
...
/*
//////////////////////////////////////////////////////////////////////
template<class T> T BSpline<T>::set_coefficient() {
    if (OK)
        if (0 <= n && n <= M)
            return s->A[n];
    return 0;
...
*/
template<class T> T BSpline<T>::getMean()
{
	return mean;
}
...
/*
//////////////////////////////////////////////////////////////////////
template<class T> T BSpline<T>::evaluate1(T x) {
    T y = 0;
    if (OK)
	{
        int n = (int)((x - xmin)/DX);
